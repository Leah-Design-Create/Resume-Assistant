...
- **未配置数据库时**：简历数据保存在项目根目录 **`data/resumes.json`**（JSON 文件），适合本地或小规模使用；后台管理页可查看、删除、上传 PDF。
- **配置数据库后**：设置环境变量 `DATABASE_URL`（PostgreSQL 连接串）后，简历与 PDF 会存入数据库，部署到 Render 等平台重启后数据不丢失，分享链接不会变回默认简历。详细配置步骤见 [docs/配置数据库持久化.md](docs/配置数据库持久化.md)。
